...
class TreeNode extends Component {
  ...
  render() {
    const { beforeNodeRender, getIcon, getUrl,getClass } = this.context.tree.props
    ...
    let nodeClass =getClass ? getClass(data) : ''
    return (
      <li className={classnames({ open })}>
        <div 
          style={indent}
          className={classnames('node-content '+nodeClass,{ active })}
        ...
        >
          ...
          {tree.props.render ? tree.props.render(data,path.length) : name}
        </div>
        {Children}
      </li>
    )
